...
### Fixed
- **CLI Execution:** Fixed quoting issues with `cmd.exe` by using `CreateProcess` directly (`shell=False`) in `session_manager.py` and strictly using `-r` flag to reuse existing windows instead of spawning duplicate app instances.
